...
**평가표는 리포트 안에 있습니다.** 리포트를 열면 출처 바로 위에 `팀 평가` 섹션이 있고,
거기 표에 한 사람당 한 줄씩 적습니다. 게임을 읽은 자리에서 바로 평가하는 구조라
따로 모아두는 파일이 없습니다.

빈 표는 빌드할 때 자동으로 들어갑니다. 점수는 이렇게 채우면 됩니다.

```html
<tbody>
  <tr><td>창연</td><td>8</td><td>9</td><td>7</td><td></td><td>6</td><td>8</td></tr>
  <tr><td>민수</td><td>7</td><td>8</td><td>7</td><td></td><td>5</td><td>7</td></tr>
</tbody>
```

- **평가자** — 사람마다 한 줄. `—` 로 남겨두면 평가하지 않은 것으로 봅니다
- **점수** — 1~10. **판단 근거가 없는 축은 비워두세요.** 그 사람의 평균에서 빠집니다
  (미출시작의 `검증` 축이 대표적입니다. 0을 넣으면 부당하게 점수가 깎입니다)

리포트 안에서 종합 평균·참여 인원·축별 평균이 바로 계산돼 표시되고,
같은 값이 메인 페이지 카드와 랭킹에도 올라갑니다.
