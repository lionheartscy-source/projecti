...
리포트 상단바의 `← 아카이브` 버튼은 **빌드할 때 자동으로 들어갑니다.** 리포트를 쓸 때
넣지 않아도 되고, 이미 있는 파일은 건드리지 않습니다. 폴더 깊이에 맞춰 경로도
알아서 맞춥니다 (`reports/kr/x.html` → `../../index.html`).

상단바가 `<a class="brand">` 구조가 아닌 리포트에는 넣지 못합니다. 그때는 빌드 로그에
경고가 뜨니 해당 파일만 손보시면 됩니다.
...
평가하는 방법이 두 가지입니다. 둘 다 쓸 수 있고, 섞어 써도 됩니다.

### ① 리포트에서 한 편씩

리포트를 읽고 바로 그 아래 `팀 평가` 에서 점수를 넣습니다. 현재 팀 점수도 같이 보입니다.

1. 이름과 점수를 넣습니다
2. **`<게임>.csv 내려받기`**
3. 받은 파일을 **`ratings/`** 폴더에 넣고 `업로드.bat`

리포트가 서버에서 그 게임의 현재 CSV 를 읽어오므로, 내려받은 파일에는
**다른 사람 점수도 함께 들어갑니다.** 덮어써서 남의 점수가 사라지지 않습니다.

### ② 평가.html 에서 몰아서

메인 페이지 오른쪽 위 `★ 평가하기` 로 들어갑니다.

1. 이름을 넣습니다
2. 게임을 눌러 6개 축을 1~10으로 매깁니다 — **판단 근거가 없는 축은 비워 두세요**
   (미출시작의 `검증` 축이 대표적입니다. 0을 넣으면 부당하게 점수가 깎입니다)
3. 다 되면 아래 **`ratings.csv 내려받기`**
4. 받은 파일을 저장소 폴더에 덮어쓰고 `업로드.bat`

입력한 내용은 그 브라우저에 자동 저장되니 나눠서 하셔도 됩니다.

**다른 사람 점수와 합치려면** 먼저 `불러오기` 로 저장소의 현재 `ratings.csv` 를 읽으세요.
남의 줄은 그대로 두고 내 줄만 더해서 내보냅니다.

> 이 페이지는 서버에 저장하지 않습니다. GitHub Pages 가 정적 사이트라 입력을 받아둘 곳이
> 없기 때문입니다. 그래서 파일을 내려받아 저장소에 넣는 단계가 남습니다.
...
| 파일 | 만드는 곳 | 범위 |
|---|---|---|
| `ratings.csv` | 평가.html | 전체 게임 |
| `ratings/<슬러그>.csv` | 리포트의 `팀 평가` | 게임 한 편 |

**같은 게임에 같은 사람이 양쪽에 있으면 `ratings/` 쪽이 이깁니다.**
리포트에서 방금 매긴 점수가 예전 일괄 파일에 덮이지 않도록 하기 위해서입니다.

결과는 **메인 카드의 점수 배지 · 랭킹 레일 · 각 리포트의 `팀 평가` 섹션** 세 곳에 반영됩니다.
...
히어로 왼쪽 랭킹 레일에는 현재 탭 기준 상위 10편이 나옵니다.

> `ratings.csv` 는 더 이상 읽지 않습니다. 남아 있다면 지우셔도 됩니다.
