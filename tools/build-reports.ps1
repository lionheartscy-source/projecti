...
$RatingBlockCss = @'
  ...
  /* 평가 페이지로 보내는 링크 */
  .rate-cta{margin:16px 0 0; padding-top:14px; border-top:1px dashed var(--line)}
  .rate-cta a{display:inline-flex; align-items:center; gap:6px; font-size:13px; font-weight:700;
    color:#fff; background:var(--accent); padding:9px 16px; border-radius:8px;
    text-decoration:none; transition:background .16s}
  .rate-cta a:hover{background:var(--accent-ink); text-decoration:none}
...
__BODY__
      ...
      <p class="rate-cta"><a href="__RATEPAGE__">평가 페이지에서 점수 매기기 →</a></p>
    </div>
  </section>
  <!-- RATING-END -->
'@
